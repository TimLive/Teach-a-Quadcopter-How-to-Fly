...
class Actor:
    ...
    def build_model(self):
        ...
        net = layers.BatchNormalization(momentum=0.99, epsilon=0.001)(net) 
...
#         optimizer = optimizers.SGD(lr=0.0003)
#         optimizer = optimizers.SGD(lr=0.0002, momentum=0.9, nesterov=True)
...
class Critic:
    ...
    def build_model(self):
        ...
        net_states = layers.BatchNormalization(momentum=0.99, epsilon=0.001)(net_states)
...
#         net_states = layers.Dense(units=32, activation='relu')(net_states)      
        ...
        net_actions = layers.BatchNormalization(momentum=0.99, epsilon=0.001)(net_actions)
...
class DDPG():
    ...
    def __init__(self, task):
        ...
        self.best_score_in_period = -np.inf
        self.best_pose = []
        self.minimum_distance = np.inf
#         self.learn_mode = False
        self.best_rewards = []
        self.avg_rewards = []
        self.rewards_in_best_period = []
    ...
    def reset_episode(self):
        ...
        #new code
        self.best_score_in_period = 0.0
        ####
        
        return state
    
    #new code
    def update_best_scores(self, reward):
        
        if reward > self.best_score_in_period:
            self.best_score_in_period = reward
            
        if reward > self.best_score:
            self.best_score = reward
            self.best_pose = copy.copy(self.task.sim.pose[:3])
            
        if self.task.distance < self.minimum_distance:
            self.minimum_distance = self.task.distance
...
#             self.learn_mode = True
        ...
        #new code
        self.update_best_scores(reward)
